...
const saveUpdate = debounce(
  (votes, lastUserVotes, blogId, completeCallback) => {
    if (votes === 1 && lastUserVotes === 0) {
      upVoteBlog(blogId).catch(e => {
        console.error('Error in upvoting', e);
      });
    } else if (votes === 0 && lastUserVotes === 1) {
      downVoteBlog(blogId).catch(e => {
        console.error('Error in downvoting', e);
      });
    }
    completeCallback();
  },
...
function useVotes(blogId, userId) {
  ...
  useEffect(() => {
    ...
    getBlogVotes(blogId)
      .then(handleBlogsVotes)
      .catch(handleBlogVotesError);
    getMyVotes(blogId)
      .then(handleMyVotes)
      .catch(handleUserBlogVotesError);
  }, [blogId, userId]);
