...
import { Button, Comment, Form, Popup } from 'semantic-ui-react';
...
const CommentComponent = ({
  ...
  const [replies = [], updateReplies, newReply, addNewReply] = useReplies(
    blogId,
    commentId
  );
  const createReplyComment = (event, replyContent) => {
    ...
    setSingleReply('');
    if (!replyContent || replyContent.trim() === '' || !_id) {
      return;
    }
    const createdTime = '...';
    const newReplyData = {
  ...
  const addReplyButton = (
    <Button
      ...
      labelPosition="right"
      icon="edit"
      onClick={event => createReplyComment(event, singleReply)}
      primary
    />
  );
  const replyInput = (
